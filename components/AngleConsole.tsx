...
function ThreeCameraPreview({ camera, output, theme, humanMode, customHumanFile, captureApiRef, onCameraChange, onFramingStatus, onNotify }: { camera: AngleCameraState; output: AngleOutputSpec; theme: 'light' | 'dark'; humanMode: HumanMode; customHumanFile: File | null; captureApiRef: MutableRefObject<GuideCaptureApi | null>; onCameraChange: (patch: CameraPatch) => void; onFramingStatus: (status: GuideFramingStatus) => void; onNotify: (message: string) => void }) {
    ...
    try {
      ...
      captureApiRef.current = {
        capture: async (captureOutput) => {
          ...
          // Export exactly the same rolled camera view shown in the output
          // frame.  Previously the live preview applied roll, while the
          // clean guide image sent as reference 2 stayed level.
          guideCamera.rotateZ(THREE.MathUtils.degToRad(state.roll));
          ...
          return { id: `angle-guide-${crypto.randomUUID()}`, name: `3D构图导引-${captureOutput.width}x${captureOutput.height}.webp`, dataUrl };
        },
      };
...
export default function AngleConsole({ theme, reference, initialCamera, initialCameraStart, models, defaultProviderId, defaultProviderName, defaultModelId, results, busy, onReferenceFiles, onExit, onRemoveReference, onBrowseHistory, onGenerate, onOpenResult, openResultId, suppressAutoOpenId, onResultOpened, onUseResult, onDownloadResult, onNotify }: AngleConsoleProps) {
  ...
  return <section className="angle-page" onPaste={handlePaste} onDragStart={(event) => { if ((event.target as HTMLElement).closest('img')) event.preventDefault(); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const files = Array.from(event.dataTransfer.files || []).filter((file) => file.type.startsWith('image/')); if (files.length) { replaceReferenceFiles(files); onNotify(`已添加 ${files.length} 张参考图`); } }}>
    <header className="angle-console-topbar">
      <div className="angle-console-brand-group"><button type="button" className="angle-brand" onClick={onExit} title="返回 SANMAO.AI"><div className="angle-logo">A</div><div><b>ANGLE CONTROL</b><small>CAMERA VIEW GENERATOR</small></div></button><button type="button" className="angle-exit-button" onClick={onExit}>← 返回 SANMAO.AI</button></div>
    ...
    <div className="angle-workspace surface">
      <div className="angle-workspace-head"><div className="angle-stage-heading"><span>REFERENCE</span><strong>原图参考 · 3D 导引</strong></div><span className="angle-stage-hint">在右侧 3D 导引中拖动调整机位 · 滚轮调整距离</span></div>
      <div className="angle-preview-grid">
        <div className="angle-preview-pane angle-reference-pane">
          ...
          {reference ? <div className="angle-image-frame"><img draggable={false} src={reference.dataUrl} alt={reference.name}/><div className="angle-reference-role">参考图 1 · 身份 / 服装 / 风格</div></div> : <div className="angle-empty-reference"><strong>粘贴、拖入或上传参考图</strong><span>也可以从生成历史中选择一张已有图片。</span><div><button type="button" className="primary-small" onClick={() => referenceInputRef.current?.click()}>选择图片</button><button type="button" className="ghost-button" onClick={onBrowseHistory}>打开历史</button></div></div>}
          ...
          <div className="angle-pane-tools"><button type="button" onClick={() => referenceInputRef.current?.click()}>{reference ? '更换参考图' : '上传参考图'}</button><button type="button" onClick={onBrowseHistory}>从历史选择</button>{reference && <button type="button" title="恢复人物居中" onClick={() => updateCamera({ frameX: 0, frameY: 0 })}>人物居中</button>}</div>
          <div className="angle-hud"><span>Camera <b>{roundViewportValue(camera.yaw)}° / {roundViewportValue(camera.pitch)}°</b></span><span>Lens <b>{roundViewportValue(camera.focal)}mm</b></span><span>Distance <b>{roundViewportValue(camera.distance)}×</b></span></div>
          {reference && <button type="button" className="angle-reference-remove" onClick={removeReference}>移除参考图</button>}
        </div>
        <div className="angle-preview-pane angle-model-pane"><div className="angle-pane-label"><b>3D 输出构图</b> · 框内画面作为参考图 2</div><ThreeCameraPreview camera={camera} output={angleOutput} theme={theme} humanMode={humanMode} customHumanFile={customHumanFile} captureApiRef={guideCaptureApiRef} onCameraChange={update3dCamera} onFramingStatus={setFramingStatus} onNotify={onNotify}/><details className="angle-guide-display"><summary>导引显示</summary><div><button type="button" className={humanMode === 'gray' ? 'active' : ''} onClick={() => setHumanMode('gray')}>中性灰模（默认）</button><button type="button" className={humanMode === 'natural' ? 'active' : ''} onClick={() => setHumanMode('natural')}>自然人物</button><button type="button" className={humanMode === 'outline' ? 'active' : ''} onClick={() => setHumanMode('outline')}>清晰轮廓</button><button type="button" className={humanMode === 'default' ? 'active' : ''} onClick={() => setHumanMode('default')}>士兵</button><button type="button" className={humanMode === 'custom' ? 'active' : ''} onClick={() => humanInputRef.current?.click()}>导入 GLB</button></div><input ref={humanInputRef} hidden type="file" accept=".glb,model/gltf-binary" onChange={(event) => { const file = event.target.files?.[0] || null; if (file) { setCustomHumanFile(file); setHumanMode('custom'); } event.currentTarget.value = ''; }}/></details><div className={`angle-framing-badge ${framingStatus.level}`}><b>{guideFramingLabel(framingStatus.level)}</b><span>{framingStatus.title}</span></div><div className="angle-three-readout"><strong>机位 {angleName(camera.yaw)} · {roundViewportValue(camera.yaw)}° / {roundViewportValue(camera.pitch)}°</strong><span>输出 {angleOutput.width}×{angleOutput.height} · {angleOutput.aspectRatio}</span></div></div>
      </div>
      <div className="angle-preset-strip">{ANGLE_PRESETS.map((preset) => <button type="button" key={preset.id} className={Math.abs(camera.yaw - preset.yaw) < 1 && Math.abs(camera.pitch - preset.pitch) < 1 ? 'active' : ''} onClick={() => applyPreset(preset.yaw, preset.pitch)}><b>{preset.label}</b><span>{preset.yaw}° / {preset.pitch}°</span></button>)}</div>
    </div>
