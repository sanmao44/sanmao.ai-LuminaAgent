...
import { MCP_MAX_SERVERS, listMcpServers, patchMcpServer, redactMcpServer, removeMcpServer } from '@/lib/mcp/store';
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await context.params;
    clearMcpToolCache(decode(id));
    if (!removeMcpServer(decode(id))) return Response.json({ error: 'MCP 服务不存在。' }, { status: 404 });
