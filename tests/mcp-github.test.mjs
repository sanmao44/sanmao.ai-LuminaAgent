...
test('真实调用的结果回写连接器状态：凭据失效记成「需要重新连接」，成功一次就清掉', async () => {
  await withDataDir(async (dataDir) => {
    // 只测「回写」这一层：服务形状按面板拿到的真实配置给，不经过 store（连接配置那份文件
    // 同时还有别的改动在做，这里不该跟着它一起红）。
    const entry = mcp.findCatalogEntry('github');
    const server = { id: 'github', name: 'GitHub', url: 'https://api.githubcopilot.com/mcp/', enabled: true, allowWrite: false, catalogId: 'github' };
