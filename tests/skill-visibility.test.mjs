...
test("an enabled skill keeps the request on the tool round so the model can really read it", () => {
  ...
  assert.match(route, /plainMessage = filesystemActionRequest \|\| browserAutomationRequest \|\| artifactGenerationRequest \|\| hasInlineToolCallMarkup\(plainMessage\)/, '工具调用文本不能被当成普通技能回答直接展示');
});
