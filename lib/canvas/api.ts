...
export async function generateCanvasVideo(input: {
  ...
  const referenceData = await Promise.all((input.references || []).slice(0, 16).map((item) => asDataUrl(item.url)));
  const firstFrame = input.inputMode === 'first-frame' || input.inputMode === 'frames' ? referenceData[0] : undefined;
  ...
  const result = await request<{ task: { id: string; status: string; progress?: number; videoUrls?: string[]; error?: string; modelId?: string } }>('/api/video/generate', {
    ...
    body: JSON.stringify({
      ...
      input: {
        ...
        ...(lastFrame ? { lastFrame } : {}),
        referenceImages: input.inputMode === 'reference' ? referenceData : input.inputMode ? [] : referenceData,
        ...(input.referenceVideo ? { referenceVideo: input.referenceVideo } : {}),
      },
    }),
  });
  return result.task;
...
export async function generateCanvasUpscale(input: {
  ...
  return request<{ images: Array<{ url: string; revisedPrompt?: string }>; model?: { id?: string; name?: string; provider?: string } }>('/api/upscale', {
    method: 'POST',
