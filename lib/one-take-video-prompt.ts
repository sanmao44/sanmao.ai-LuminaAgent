...
最终输出规则：内部完成分析后，只输出一个可直接交给 Seedance 2.0 使用的完整视频生成 Prompt，不要输出逐图分析过程、解释、免责声明或多套方案。必须严格使用以下结构，并根据实际图片数量补充所有转场：
...
### X–Xs
描述 Image 2 对应的连续镜头。按图片数量继续写后续时间段和 Transition N。
...
### Visual Continuity
强调 same character identity, coherent environment, consistent costume, stable facial features, physically plausible motion, realistic parallax, continuous lighting, continuous camera direction, cinematic depth of field。

### Negative Constraints
加入针对当前画面的负面约束，重点防止人物漂移、物体变形、空间跳变、镜头断裂、硬切、跳帧、突兀换景、镜头抖动和 AI 伪影。

最重要原则：不要把参考图片理解为几张需要依次展示的幻灯片，而要把它们理解为同一个连续世界中的几个关键视觉节点；先在内部回答“如果这是真实电影拍摄，摄影机怎样才能从第一张图所在的位置，真实地连续移动到最后一张图的位置？”，再输出唯一的 15 秒、无剪辑、无跳变、丝滑、自然、合理、惊艳、电影级的一镜到底视频 Prompt。
`.trim();
