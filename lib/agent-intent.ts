...
const documentDeliverablePattern = /(?:word|docx|excel|xlsx|ppt|pptx|pdf|文档|文件|简历|报告|周报|日报|月报|纪要|会议记录|总结|汇报|方案|合同|论文|说明书|手册|报表|台账|数据表|幻灯片|演示文稿|演示|deck|模板)/i;
...
export function classifyAgentDeliverable(input: string, context: AgentIntentContext = {}): AgentIntentDecision {
  ...
  const asksForImageWithoutTarget = /(?:画(?:个|一?只|一个|一张|一幅|一?条|一?头|一?匹|一?朵|一?辆|一?艘|一?座|一?棵|一?位)|画出|出图|生图|生成一张|生成一个|做一张|做一个|做个|来一张|来个).{1,80}/i.test(text) && !documentDeliverablePattern.test(text) && (!textArtifactPattern.test(text) || embeddedTextPattern.test(text));
