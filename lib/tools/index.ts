...
/** 按运行时 id 找工具；审计和日志认 id，不认会变的名字。 */
export function findToolById(id: unknown): ToolDefinition | null {
  return TOOL_BY_ID.get(String(id || '')) || null;
}
...
export function toolRisk(name: unknown, extraTools: readonly ToolDefinition[] = []) {
  return findToolDefinition(name, extraTools)?.risk || null;
}
