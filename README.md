...
启动器会自动检查 Node.js 20.9+、安装依赖、构建项目、使用 SANMAO.AI 专用的 3210～3220 端口段并打开浏览器。重复双击不会启动第二个后台服务，而是打开已有网页；关闭最后一个网页后，服务会在心跳租约过期后自动退出（通常约 10～15 秒）。
...
- 新增服务端签名媒体地址，为 Agnes 访问本地图片、视频和音频提供短期安全 URL；未配置公网地址时不会自动暴露内网。
