...
启动器会自动检查 Node.js 20.9+、安装依赖、构建项目、使用 SANMAO.AI 专用的 3210～3220 端口段并打开浏览器。只有检测到已保存且有访问密钥的 Agnes 服务商时，现有启动入口才会按需准备免费的临时 HTTPS 通道：首次使用可能自动下载 Cloudflare 通道组件，不需要服务器、域名、`.env` 或手工配置。没有 Agnes 配置时不会下载或启动中转；其他服务商也保持原有启动行为。重复双击不会启动第二个后台服务，而是打开已有网页；关闭最后一个网页后，服务会在心跳租约过期后自动退出（通常约 10～15 秒），临时通道也会随服务停止而清理。
...
- Agnes 图生视频支持本地图片自动短期中转，普通用户无需配置公网域名；图片地址约 30 分钟后自动失效和清理。Windows 本机、Windows 局域网、macOS 和 Linux 的原有启动器都会复用这个能力，但只有检测到 Agnes 配置时才启用临时通道。
...
普通用户直接上传图片即可，应用会自动完成图片压缩和临时中转，不需要编辑 `.env`、配置 HTTPS 或重启应用。外部公网图片 URL 会直接使用，不会重复上传。

如果自动中转服务暂时不可用，任务会在提交 Agnes 前停止，不会创建无效任务或扣除视频额度；文本和普通图片不受影响。高级用户仍可使用自己的公网媒体地址，具体部署方式见 [`relay/README.md`](relay/README.md) 和 `.env.example`。
...
Agnes 云端必须能通过公网 HTTPS 读取图生视频参考图。项目提供独立的中转部署包，详见 [`relay/README.md`](relay/README.md)。部署完成后，将中转域名作为发布版本的默认配置；普通用户端无需填写任何地址。
# SeedVR2-7B 图片超分
